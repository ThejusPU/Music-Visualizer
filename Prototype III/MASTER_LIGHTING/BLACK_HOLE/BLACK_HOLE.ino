...
void starsOnStrip() {
  ...
  for (int x = 0; x < NUM_LEDS; x++) {
    st = checkColoring(x);
    if (stars[x] == 0 && st != -1) {
      int colorPos = color + offset;
      if (colorPos >= NUM_COLORS) colorPos = color + (NUM_LEDS - x);
      float percent = ((float)st) / (2 * (float)maxStar[x]);
      leds[x] = CRGB(25 + reds[colorPos] * percent, 25 + blues[colorPos] * percent, 25 + greens[colorPos] * percent);
    }
    else if (stars[x] == 0) {
      leds[x] = CRGB(25, 25, 25);
    }
    else {
      leds[x] = CRGB(0, 0, 0);
    }
