...
void loop() {
  ...
  for(int i = NUM_LEDS - 1; i >= (NUM_LEDS/2)+updateLEDS; i--) {
    leds[i] = leds[i - updateLEDS];
  }
  for(int i = 0; i <= (NUM_LEDS/2)-updateLEDS+1; i++) {
    leds[i] = leds[i + updateLEDS];
  }
  ...
  for(int i = (NUM_LEDS/2)-updateLEDS+1; i <= (NUM_LEDS/2)+updateLEDS; i++) {
...
Color pitchConv(int p, int b) {
  ...
  setColor(&c, light.r * bright, light.g * bright, light.b * bright);
  return c;
}
