...
void setup() { 
    ...
    for(int i = 0; i < NUM_LEDS ; i++) {
      if(i < NUM_LEDS/2){
        int green = setLeds(i);
        leds[i] = CRGB(1-green,green,0);
      }
      else{
        int blue = setLeds(i-NUM_LEDS);
        leds[i] = CRGB(0,1-blue,blue);
      }
    }
    FastLED.show();
}
...
//  int randon = analogRead(RANDON_PIN);
...
//  Serial.print(randon);
//  Serial.print("\n");
//  Color nc = pitchConv(newPitch, bright);
//
